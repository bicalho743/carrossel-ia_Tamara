...
try:
    from brand_assets import apply_brand
    HAS_BRAND = True
except ImportError:
    HAS_BRAND = False
...
def main():
    ...
    with sync_playwright() as p:
        ...
        for num, nome, data in slides:
            ...
            # sobrepor logo oficial + padronagem (Pillow)
            if HAS_BRAND:
                is_dark = layout.get("fundo") == "surface_dark"
                slide_type = (meta.get("tipo") or "").lower()
                stype = "capa" if "capa" in slide_type else ("cta" if "cta" in slide_type else "")
                apply_brand(str(fname), is_dark=is_dark, slide_type=stype, scale=args.scale)
            print(f"  ✓ {fname.name}")
        browser.close()
